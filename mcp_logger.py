...
_text_logger = logging.getLogger("mcp_audit")
_text_logger.setLevel(logging.INFO)

if not _text_logger.handlers:
    _fh = logging.FileHandler(TEXT_FILE, encoding="utf-8")
...
def log_tool_call(
    ...
    if patient_id:
        extra += f"  patient={patient_id}"

    _text_logger.info(f"CALL  {tool_name:<30}  call_id={call_id}{extra}")
    return call_id
...
def log_session_start(
    ...
    _text_logger.info(f"SESSION START  session_id={session_id}{extra}")
...
def log_session_end(
    ...
    _text_logger.info(f"SESSION END    session_id={session_id}{extra}")
