# llm.py — MedGemma client via local hugging face model
...
async def ask_medgemma(prompt: str, patient_context: str = "") -> str:
    """
    Send a prompt to MedGemma and return the response text.
    ...
    try:
        ...
        raw_text = _tokenizer.decode(generation, skip_special_tokens=False)
        print(f"[DEBUG DECODE NO SKIP]: {repr(raw_text)}")
        decoded = _tokenizer.decode(generation, skip_special_tokens=True).strip()
        print("INPUT LEN:", input_len)
        print("OUTPUT SHAPE:", generation.shape)
        return decoded
...
JSON:"""
    ...
    return {
        "action": "respond",
        "message": raw.strip() or "I'm sorry, I couldn't understand that. Could you please rephrase?"
