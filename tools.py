...
def doctor_create_medical_record(
    ...
    """
    Doctor-only: create a medical record for a patient.

    If appointment_id is provided, it must:
    - exist
    - belong to the acting doctor
    - belong to the given patient

    It still allows a doctor to create a record for any patient without an appointment id. We can add stricter checks later if needed.
    """
    ...
    if linked_appointment_id:
        ...
        if appointment["patient_id"] != patient["id"]:
            return {
                "success": False,
                "reason": "Appointment does not belong to this patient.",
            }
